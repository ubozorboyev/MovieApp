...
import ubr.persanal.movieapp.data.model.MovieDataByActor
...
class MovieByActorAdapter(private val baseInterface: BaseInterface) :
    ...
    private val dataList = arrayListOf<MovieDataByActor>()

    inner class ViewHolderMovieActor(private val itemBinding: ItemMovieActorBinding) :
        RecyclerView.ViewHolder(itemBinding.root) {

        fun bind(data: MovieDataByActor) {
            itemBinding.movieName.text = data.title
            Glide.with(itemView).load(Common.IMAGE_URL + data.poster_path)
